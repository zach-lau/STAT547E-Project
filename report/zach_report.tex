...
makes use of the concept of a Schrodinger bridge
...
This report builds up intuition about the Schrodinger bridge problem from the
...
the Schrodinger bridge.  We then move onto mixtures of Brownian bridges which
...
recover a finite version of the Schrodinger bridge problem. In the limit of
infinite particles, this approaches exactly the standard Schrodinger bridge
...
particle $i$ ends up at location $j$ and $0$ otherwise (i.e.
...
discovered the concept of a Schrodinger Bridge. Another interesting point,
we make is to recall that the cross entropy corresponds to the cost function
from the Monge-Kantorovich problem. Thus this optimizaiton corresponds
to an \emph{entropy-regularized} version of the Monge-Kantorovich problem.
Changing variance of the Brownian motion changes the strength of
regularization.
...
\begin{enumerate}
    ...
    This amounts to learning the score \cite{song2020generativemodelingestimatinggradients}
    \cite{ho2020denoisingdiffusionprobabilisticmodels}
\end{enumerate}
...
bit-masked swap proposals and random pairings. Stationarity is maintained
since each individual swap proposal is reversible (after MH correction). The
target is log concave \cite{léonard2013surveyschrodingerproblemconnections},
therefore multiomdality is not an issue. An
...
\begin{algorithmic}[1]
\Require $M = 2^N$ particles with fixed positions $x_1,\ldots,x_M \sim r$,
         initial $y$-values $y_1,\ldots,y_M$ with empirical marginal $c$,
         regularisation $\varepsilon > 0$
\State $\mathit{bit} \gets 0$
\Loop
    ...
    \For{$i = 0, 1, \ldots, M-1$ such that $(i \gg \mathit{bit}) \mathbin{\&} 1 = 0$}
        ...
        \If{$\Delta E \leq 0$ \textbf{or} $U \sim \mathrm{Uniform}(0,1) < e^{-\Delta E / \varepsilon}$}
            \State Swap $y_i \leftrightarrow y_j$
        \EndIf
    \EndFor
    ...
    \If{$\mathit{bit} = 0$} \Comment{End of full $N$-bit cycle}
        ...
        \For{$k = 1, 3, 5, \ldots, M-1$} \Comment{Consecutive pairs}
            \State Propose swap between particles $\sigma(k)$ and $\sigma(k+1)$ via MH
        \EndFor
    \EndIf
\EndLoop
\end{algorithmic}
...
\section{Conclusion}
In this project we have built up a deeper understanding of the Schrodinger
...
the reference dynamcis in finite particle systems.
