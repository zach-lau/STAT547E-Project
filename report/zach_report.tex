...
and the optimal transport cost functio (see figure \ref{fig:epsilon_effect}).
\begin{align}
...
proposing swaps of the respective coordinate, then shuffles the indices. This
ensures both global communication between particles, as well as breaking
...
values of the two interacting particles). The overall kernel can be seen as
...
naturally induces entropy regularization. The setup for this algorithm is
slightly different, in that particles are sampled from the marginals, and thus
do not necessarily at exactly the same location. However qualitiatively and
...
from the marginals, which may be significantly more information dense.
Furthermore, the sampling problem is extremely well-behaved; we have a
log-concave target and uniform ergodicity (since we are working on finite state
space).
