...
\begin{document}
\title{STAT 547E - Report - Diffusion Schrodinger Bridge}
\author{Zachary Lau}
\date{\today}
\maketitle

\begin{abstract}
The Schrodinger Bridge problem is a classic problem in physics, which has
received renewed attention in statistics thanks to its application to diffusion
models. This report provides an accesible introduction to the Schrodinger bridge
problem for statisticians, discusses its connection to diffusion model, and
compares 
\end{abstract}
...
The Brownian Bridge can be seen as the precursor problem of the Schrodinger
...
motion.  Furthermore as Schrodinger points out in his original paper, the
...
\end{align}
On the other hand looking at the particles as a collective, we might be
...
the pairing by $(\mathbf i, \mathbf j)$ and, 
taking this product over all $i$ points, we get the posterior over pairings by
\begin{align}
    \mathbf p(\mathbf i,\mathbf j) \propto \prod_{(i,j)} \exp\left(-\frac 1 2(a_i-b_j)^2\right)
\end{align}
(This pairing as written is actually not identifiable, to solve this we can
simply fix $\mathbf i = [1, 2, 3, \ldots, N]$). We make an interesting physical
remark that will become important later. Supposing that we associated
$p$ with the counting measure on $[N] \times [N]$ where $[n] = \{1, \ldots N\}$
such that $p(i,j)=1$ if and only if particle $i$ ends up at location $j$ (i.e.
it absorbs all the information about which particle goes where). Let
$q$ be the reference measure on $[N] \times [N]$ given by
$q(i,j) = \exp\left(-\frac 1 2(a_i-b_j)^2\right)$. Then $q$ contains all the
information about how Brownian motion actually works (i.e. about the underlying
system). Then the log posterior actually represents the negative cross-entropy 
between $p$ and $q$. That is
\begin{align}
    -H(p,q) &= \int \log q(i,j) \d p \\
    &= \frac 1 N \sum_{(i,j)} -\frac 1 2 (a_i - b_j)^2 \\
    &= \frac 1 N \log \mathbf p(\mathbf i, \mathbf j) \\
    \log \mathbf p(\mathbf i, \mathbf j) &= -NH(p,q)
\end{align}
Already, we can see an interesting phenomenon. Notice that multiplying the
log-likelihood by $N$ corresponds to a \emph{power posterior}. Therefore for
large $N$ we might expect the posterior to concentrate near the minimizer of
the cross-entropy.
...
$p$ from before to such that $p(i,j)$ is the \emph{number of particles} starting
...
$p$-as a matrix. In this case let the matrix be given by $P_{i,j} = p(i,j)$.

Firstly we must constrain the support such that the each row and column of 
$P$ sums to $M$. These are our \emph{marginal} constraints. We next must
...
our marginal constraints. Note there is a distinction between pairings and $P$
matrices. For example if we swap the labels on two particles we have a new
pairing, but the $P$ matrix is unchanged! Indeed we have discovered the
difference between a microstate (pairing) and macro-state (P-matrix).

For our $P$ matrix this has \emph{combinatorial}
implications. The total number of pairings correponding to a $P$ matrix is
given by a multinomial, namely
\begin{align}
    \frac{(NM)!}{\prod_{i,j} P_{i,j}!}
\end{align}
Our prior over $P$ matrices has this form. Taking into account
the Brownian Noise likelihood from early, we get the posterior
\begin{align}
    p(P) \propto \frac{1}{\prod_{i,j} P_{i,j}}!\prod_{(i,j)} \exp\left(-\frac 1 2(a_i-b_j)^2\right) \\
    \log p(P) = - \sum_{i,j} \log P_{i,j}! + \sum_{(i,j)} - \frac 1 2 P_{i,j}(a_i - b_j)^2 + \log Z
\end{align}
Where $Z$ is a normalizing constant. Viewing $P$ as a probability measure, and using $q$
to denote the reference measure from before, the second term becomes a multiple
of the cross entropy, namely $- NM H(p,q)$.
Using Stirling's approximation, the first term becomes 
\begin{align}
    \sum_{i,j} - P_{i,j} \log P_{i,j} + P_{i,j} + O(\log P_{i,j})
\end{align}
On practical scales, the first two terms are the most significant.
Normalizing by the total $NM$ particles,
we note that this approaches the \emph{entropy} (in the statistics sense) of the probability measure 
version of $P$ plus a constant
\begin{align}
    \sum_{i,j} -P_{i,j} \log P_{i,j} + P_{i,j} &=
        NM H(p) - NM \log (NM) + NM
\end{align}
Thus our large $M$ approximation gives the posterior as 
\begin{align}
    \log p(P) &= NM (H(p) - H(p,q)) + NM - NM \log (NM) \\
    &= NM KL(p | q) +  NM - HM \log (NM)
\end{align}
...
distribution on $P$ collapses to the delta distribution at the minimizer of the
...
discovered the concept of a Schrodinger Bridge.

\section{The Schrodinger Bridge Problem}
Armed with a deep understanding of Brownian bridges, we are now ready to
state the Schrodinger bridge problem as the continous version of our previous
mixture of Brownian bridges problem. Briefly, given a Riemannian manifold
$\mathcal X$, the (static) Schrodinger bridge problem is to find the joint
distribution $P$ on $\mathcal X \times \mathcal X$ which
\begin{enumerate}
    \item Satisfies the marginal constraints $X_0 \sim \mu$ and $X_1 \sim \nu$
    \item Minimizes the
KL-divergence to the reference Brownian motion on $\mathcal X \times \mathcal X$
\end{enumerate}
In the general case, the reference Brownian motion is given by
$Q(x_0, x_1) \propto \exp\left(-\frac 1 2 (x_1 - x_0)^2 \right)$. The reference
is always a measure, but it may not be a probability measure. From this
formulation the connection to \emph{optimal transport} becomes clear. Once
a transport plan is found, the distribution on paths is just a mixture of
Brownian bridges with weights given by the transport plan.

\section{Schrodinger Bridge to Diffusion Models}
Before discussing how to actually compute the Schrodinger bridge, we touch on
...
\begin{enumerate}
    ...
    This amounts to learning the score (CITE ALL THE LITERATURE)
\end{enumerate}
...
Solving the Schrodinger bridge problem helps us address this discomfort.
...
constraints. Note that this means we no longer need to specify a drift term
in our forward process, the drift term is determined by the Schrodinger bridge
problem to ensure that the prior is reached \emph{in finite time}. This is the
first key idea in \cite{debortoli2023diffusionschrodingerbridgeapplications}.

\section{Solving the Schrodinger Bridge Problem}
We now return to the problem of actually solving the Schrodinger bridge problem.
We will focus again on a finite set. First we will introduce the classic
...
the Sinkhorn algorithm is very simple. Letting $P$ be transport matrix
normalized by the total number of particles. The Sinkhorn algorithm proceeds
as follows
\begin{enumerate}
    \item Initialize $P$ at the reference distribution
    ...
    \end{enumerate}
\end{enumerate}
It is a remarkable fact shown in \cite{Sinkhorn1964} that this algoritihm
converges to the minimum entropy solution which satisfies the marginal
constraints! We will not provide a proof of convergence here, but we will
provide some intuition. It may be helpful to think of this algorithm in terms of
conditional distributions. Let rows correspond to
$X_0$ with target marginal $\mu$ and columns to $X_1$ with target marginal
$\nu$. The time we rescale the rows corresponds to computing the joint
distribution specified by $\mu$ and the reference dynamics. This joint
distribution implies a \emph{backwards} conditional distribution $X_0|X_1$.
Crucially, if we now reweight the columns, the reverse conditional distribution
is maintained and the $X_1$ marginal is adjusted. However, the forward
conditional $X_1 | X_0$ and $X_0$ marginal will now be perturbed. The algorithm
...
At a high-level the algorithm is as followed
\begin{enumerate}
...
\section{Experiments}
\subsection{Comparison with Annealing}


\subsection{Comparison with Annealed Langevin Diffusion}

\subsection{Stat mech simulation}
